'use client'
import { signIn } from 'next-auth/react'

const GitHubButton = () => {
    return (
        <button
            className="flex w-full h-14 justify-center items-center gap-5 rounded bg-white text-sm font-bold drop-shadow-md hover:bg-gray-50 duration-200"
            onClick={() => signIn('github')}
        >
            <DiscordLogo />
            <p>Sign in with Discord</p>
        </button>
    )
}

export default GitHubButton

const DiscordLogo = (props: any) => (
    <svg
        width="36"
        height="36"
        xmlns="http://www.w3.org/2000/svg"
        fill="none"
        viewBox="0 0 32 32"
        {...props}
    >
        <path
            fill="#fff"
            d="M2 11.6c0-3.36 0-5.04.654-6.324a6 6 0 0 1 2.622-2.622C6.56 2 8.24 2 11.6 2h8.8c3.36 0 5.04 0 6.324.654a6 6 0 0 1 2.622 2.622C30 6.56 30 8.24 30 11.6v8.8c0 3.36 0 5.04-.654 6.324a6 6 0 0 1-2.622 2.622C25.44 30 23.76 30 20.4 30h-8.8c-3.36 0-5.04 0-6.324-.654a6 6 0 0 1-2.622-2.622C2 25.44 2 23.76 2 20.4v-8.8Z"
        />
        <path
            fill="#5865F2"
            d="M23.636 9.34A18.762 18.762 0 0 0 19.097 8c-.195.332-.424.779-.581 1.134a17.727 17.727 0 0 0-5.03 0A11.828 11.828 0 0 0 12.897 8a18.697 18.697 0 0 0-4.542 1.343c-2.872 4.078-3.65 8.055-3.262 11.975a18.608 18.608 0 0 0 5.567 2.68c.448-.58.848-1.195 1.192-1.844a12.045 12.045 0 0 1-1.877-.859 9.19 9.19 0 0 0 .46-.342c3.62 1.59 7.553 1.59 11.13 0 .15.118.304.233.46.342-.595.337-1.225.626-1.88.86.343.648.742 1.265 1.191 1.845a18.574 18.574 0 0 0 5.57-2.682c.457-4.544-.78-8.485-3.27-11.978Zm-11.29 9.567c-1.087 0-1.978-.953-1.978-2.113 0-1.16.872-2.116 1.977-2.116 1.106 0 1.997.953 1.978 2.116.002 1.16-.872 2.113-1.978 2.113Zm7.308 0c-1.086 0-1.977-.953-1.977-2.113 0-1.16.872-2.116 1.977-2.116 1.106 0 1.997.953 1.978 2.116 0 1.16-.872 2.113-1.978 2.113Z"
        />
    </svg>
)
